...
class CommandService extends BrowserEventEmitter {
  constructor() {
    ...
    this.aiService = null // Will be instantiated during initialization
  }
  ...
  registerAICommands() {
    ...
    this.register('ai.complete.code', 'AI: Complete Code', async (context) => {
      const completion = await this.aiService.getCodeCompletion(context)
    ...
    this.register('ai.refactor.code', 'AI: Refactor Code', async (context) => {
      ...
      const refactored = await this.aiService.refactorCode(selectedCode, language, 'optimize')
    ...
    this.register('ai.analyze.code', 'AI: Analyze Code', async (context) => {
      ...
      const analysis = await this.aiService.analyzeCode(selectedCode, language)
    ...
    this.register('ai.fix.errors', 'AI: Fix Errors', async (context) => {
      ...
      const response = await this.aiService.chatWithAI(fixPrompt, { type: 'error-fixing' })
      this.emit('ai.chat.response', response)
    }, 'Ctrl+K F')

    this.register('ai.optimize.performance', 'AI: Optimize Performance', async (context) => {
      const { selectedCode, language } = context || {}
      const refactored = await this.aiService.refactorCode(selectedCode, language, 'performance')
      this.emit('ai.refactor.result', refactored)
    }, 'Ctrl+K O')
    ...
    this.register('ai.generate.tests', 'AI: Generate Tests', async (context) => {
      ...
      const generated = await this.aiService.generateCode(prompt, language, { type: 'unit-tests' })
    ...
    this.register('ai.generate.docs', 'AI: Generate Documentation', async (context) => {
      ...
      const response = await this.aiService.chatWithAI(prompt, { type: 'documentation' })
      this.emit('ai.chat.response', response)
    }, 'Ctrl+K D')
  }
