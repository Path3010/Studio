...
import { configureMonaco, getLanguageFromFilename, getAdvancedEditorOptions } from '../config/monaco_advanced'
...
const Workbench = () => {
  ...
  // Enhanced collaboration and real-time state
...
function runDemo() {
  ...
  const initializeWorkbench = async () => {
    try {
      console.log('[Workbench] Initializing enhanced AI-powered IDE...')
      
      // Configure Monaco Editor with advanced features
      if (!window.monacoConfigured) {
        configureMonaco()
        window.monacoConfigured = true
        console.log('[Workbench] Monaco Editor configured with advanced features')
      }
      
      // Initialize Socket.IO for real-time collaboration
      const socketConfig = API_CONFIG.SOCKET_CONFIG
      const socketInstance = io(socketConfig.url, socketConfig.options)
      
      socketInstance.on('connect', () => {
        console.log('[Socket] Connected to enhanced backend')
        setIsConnected(true)
        // Join the current project room for collaboration
        socketInstance.emit('join:project', 'default')
      })
      ...
      socketInstance.on('file:changed', (data) => {
        console.log('[Socket] File changed by collaborator:', data)
        // Handle real-time file updates here
      })
      
      socketInstance.on('user:joined', (data) => {
        console.log('[Socket] User joined collaboration:', data.socketId)
      })
      
      socketInstance.on('user:left', (data) => {
        console.log('[Socket] User left collaboration:', data.socketId)
      })
      
      socketRef.current = socketInstance
      
      // Initialize Enhanced Service Collection
      ...
      // Initialize Enhanced File Service with v2 API
      const fileService = new FileService()
      fileService.setSocket(socketInstance) // Enable real-time features
      await fileService.initialize()
      fileServiceRef.current = fileService
      
      // Initialize Enhanced JavaScript Execution Service with multi-language support
      const executionService = new JavaScriptExecutionService()
      executionService.setSocket(socketInstance) // Enable execution sharing
      await executionService.initialize()
      executionServiceRef.current = executionService
      
      // Register services with enhanced capabilities
      ...
      instantiationService.registerService(ServiceIdentifiers.EXECUTION_SERVICE, executionService)
      
      // Initialize Enhanced AI Service with multiple providers
      const aiService = new AIService()
      aiService.setSocket(socketInstance) // Enable AI activity sharing
      await aiService.initialize()
      aiServiceRef.current = aiService
      instantiationService.registerService(ServiceIdentifiers.AI_SERVICE, aiService)
      
      // Initialize Command Service (singleton)
